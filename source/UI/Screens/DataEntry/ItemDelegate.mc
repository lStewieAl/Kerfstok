    ...
    if (clickEvent.getType() == CLICK_TYPE_TAP) {
      ...
      switch (id) {
        ...
        case 1:
          ...
          switch (el) {
            case 0:
              getnum(new dayChange(val), "Day");
              break;
            case 1:
              WatchUi.pushView(
                new MonthView(),
                new MonthDelegate(val),
                WatchUi.SLIDE_IMMEDIATE
              );
              break;
            case 2:
              getnum(new yearChange(val), "Year");
              break;
          }
          break;
        case 2:
          ...
          switch (el) {
            case 0:
              WatchUi.pushView(
                new VarView(),
                new VarDelegate(new varChange(val)),
                WatchUi.SLIDE_IMMEDIATE
              );
              break;

            case 1:
              getnum(new datChange(val), vars[val[2]]);
              break;
          }
          break;
        case 3:
            ...
            case 0:
              {
                ...
                WatchUi.pushView(
                  dialog,
                  new DeleteConfirmationDelegate(numbase, datanum),
                  WatchUi.SLIDE_IMMEDIATE
                );
              }
              break;
            case 1:
              if (val[0] != oldval[0]) {
                if (!testtime(numbase, datanum, val[0])) {
                  ...
                  WatchUi.pushView(
                    dialog,
                    new FarWayConfirmationDelegate(numbase, datanum, val),
                    WatchUi.SLIDE_IMMEDIATE
                  );
