...
function drawarrow(dc, width, height, xorg, yorg, density) {
  ...
  dc.fillPolygon(
    [
      [sx1, sy1] as Array<Float>,
      [getx, gety] as Array<Float>,
      [sx2, sy2] as Array<Float>,
      [xtus, ytus] as Array<Float>,
...
class GlucoseView extends WatchUi.View {
  function initialize() {
    linexpos = venusq2 ? width * 0.22 : width * 0.3;
    View.initialize();
  }

  var heartfont =
    fenixe || fenix8 ? Gfx.FONT_NUMBER_MILD : Gfx.FONT_NUMBER_MEDIUM;
  var heartheight;
  var timeoff = 0;
  var datey;
  function onLayout(dc) {
    ...
    datey =
      (edge1040 || edgeexplore2 ? height * 0.05 : 0) +
      (theight * 0.8 + timeoff);
  }
  ...
  function onUpdate(dc) {
    ...
    dc.drawText(
      ...
      datey,
      Gfx.FONT_TINY,
      myTime.day_of_week + " " + myTime.day + " " + myTime.month,
      Gfx.TEXT_JUSTIFY_CENTER
    );
    var load = System.getSystemStats().battery;
    dc.drawText(
      width * 0.95,
      height * (fr965 ? 0.37 : 0.33),
      Gfx.FONT_XTINY,
      load.format("%.0f") + "%",
      Gfx.TEXT_JUSTIFY_VCENTER | Gfx.TEXT_JUSTIFY_RIGHT
    );
