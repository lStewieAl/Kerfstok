...
function getval(base, id) {
  return Storage.getValue(instorage(base, id));
}
...
function storeReceivedDataChunk(base, begin, end, ar) {
  ...
  if (
    lowest == null ||
    oldend < begin ||
    end - lowest > maxstorage ||
    (lowest >= begin && lowest < end)
  ) {
...
function deletethrough(base, datanum) {
  delval(base, datanum);
  if (datanum == storageid[base] - 1) {
    // is it then made clear that data has been removed?
    var grens = storageid[base] > maxstorage ? storageid[base] - maxstorage : 0;
